...
def _orders_axis_max(counts, limit: int) -> float:
    """
    Верх шкалы «Заявки»: по данным, чтобы столбики и подписи были читаемы.
    Лимит 250 не растягивает ось, если фактический максимум намного ниже.
    """
    peak = float(pd.to_numeric(pd.Series(counts), errors="coerce").fillna(0).max() or 0)
    data_top = max(peak * 2.4, peak + 4, 12.0)
    if limit > 0 and limit <= data_top * 1.35:
        return float(max(limit * 1.08, data_top))
    return float(data_top)


_ORDERS_TEXTFONT = dict(size=14, color="#145A32", family="Arial, sans-serif")


def _add_proliv_markers(
    fig: go.Figure,
    codes,
    prolivs: pd.Series,
    *,
    ...
    """Кружки с числом проливов по центру панели (без пунктира), правая шкала Y."""
    label_x: list = []
    label_text: list = []
    hover_text: list = []
    max_p = 0
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(size=24, color="#7D3C98", symbol="circle"),
    ...
    fig.update_yaxes(
        ...
        col=col,
        secondary_y=True,
        range=[0, max(max_p * 1.2, y_mid * 2.0, 3.0)],
        showgrid=False,
...
def _fig_split_panels(
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            "Ваши заявки, шт · фиолетовые кружки — проливы (шкала справа)",
            "Залив базиса, т",
        ),
        specs=[[{"secondary_y": True}], [{}]],
    )
    y_max = _orders_axis_max(merged["Количество"], limit)
    fig.add_trace(
        go.Bar(
            x=codes,
            y=merged["Количество"],
    ...
    _add_proliv_markers(fig, codes, merged["Проливов"], row=1, col=1)
    if limit <= y_max:
        fig.add_hline(
            y=limit,
            line_dash="dash",
...
def _fig_dual_axis(
    ...
    fig.update_layout(
        ...
        height=_chart_height(n, per_row=24, base=140),
        xaxis_title="Код инструмента",
        yaxis=dict(title="Количество заявок", range=[0, y_max]),
        yaxis2=dict(
            title="Залив, т",
            overlaying="y",
            side="right",
