...
def render_tab_contract_lag(df_orders, df_contracts, max_lag_sec: float) -> None:
    ...
    st.caption(
        ...
        "Для вашей задачи главный показатель: когда **время заявки позже времени договора**. "
        "Это и есть оценка, сколько времени ушло на отправку/доставку заявки до регистрации в системе."
    )
    ...
    c4.metric("Заявка позже договора", meta.get("late_orders", 0))
    ...
    if after_count:
        ...
        best_row = (
            matched[matched["Тип сопоставления"] == "заявка после договора"]
            .assign(_delay=lambda d: d["Задержка после договора, мс"])
            .sort_values("_delay")
            .iloc[0]
        )
        st.success(
            "Лучший найденный пример: "
            f"договор **{best_row['Время договора']}**, ваша заявка **{best_row['Время заявки']}**. "
            f"Задержка **{fmt_ms_and_sec(best_row['Задержка после договора, мс'])}**."
        )
        st.plotly_chart(fig_lag_histogram(matched), use_container_width=True)
    else:
        st.info(
            "Не найдено случаев, где ваша заявка зарегистрирована после договора. "
            "Либо в этих сопоставлениях вы были раньше сделки, либо пары не нашлись по инструменту/цене/лотам."
        )

    if not matched.empty:
        st.markdown("##### Таблица сопоставлений")
        table = matched.copy()
        table["Разница, с"] = table["Разница, мс"].map(lambda v: round(float(v) / 1000, 3))
        table["Пояснение"] = table["Тип сопоставления"].map(
            {
                "заявка после договора": "Это ваш целевой кейс: сделка уже была, заявка пришла позже.",
                "заявка раньше договора": "Заявка была в системе до момента сделки.",
            }
        )
        st.dataframe(
            table.sort_values(["Тип сопоставления", "Разница, мс"], ascending=[True, True]),
    ...
    with st.expander("Как читать результат"):
        st.markdown(
            """
- Если видите договор `11:35:40.830`, а ваша заявка зарегистрирована `11:35:41.300`, то разница составляет **470 мс (0.470 с)**.
- Именно такие случаи **«заявка после договора»** и показывают ориентир: сколько времени не хватило, чтобы заявка оказалась в системе раньше сделки.
- Чем меньше эта задержка, тем ближе вы к цели. Ваш ориентир здесь — **минимум** и нижняя часть точек на графике.
- Случаи **«заявка раньше договора»** тоже полезны: они показывают, что заявка уже была в системе до момента сделки.
- Это оценка по временам из выгрузок терминала, а не прямой замер сети/кнопки/биржи.
            """
        )
